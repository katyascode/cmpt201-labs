...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  // Search for key in output array
  for (int i = 0; i < *result_count; i++) {
    if (output[i].doubled_value == input->doubled_value) {
      output[i].line_numbers[output[i].count] =
          input->line_number; // Append line number
      output[i].count++;      // Increment count
      return;
    }
  }

  // Case: not found - create a new entry and update number of unique keys
  int idx = *result_count;
  output[idx].doubled_value = input->doubled_value;
  output[idx].line_numbers[0] = input->line_number;
  output[idx].count = 1;
  (*result_count)++;
}

void reduce(Output *output) {
  // Simply print in format: (key, [line0, ... , lineN])
